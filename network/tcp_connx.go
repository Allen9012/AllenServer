...
func NewTcpConnX(conn *net.TCPConn, msgBuffSize int) (*TcpConnX, error) {
	tcpConn := &TcpConnX{
		closed:      -1,
		verify:      0,
...
func (c *TcpConnX) AsyncSend(msgID uint64, msg interface{}) bool {
...
func (c *TcpConnX) AsyncSendLastPacket(msgID uint64, msg interface{}) bool {
	data, err := c.msgParser.Pack(msgID, msg)
	if err != nil {
...
func (c *TcpConnX) OnConnect() {
	logger.Debug("[OnConnect] 建立连接 local:%s remote:%s", c.LocalAddr(), c.RemoteAddr())
}

func (c *TcpConnX) OnClose() {
	logger.Info("[OnConnect] 断开连接 local:%s remote:%s", c.LocalAddr(), c.RemoteAddr())
}
