...
func ExampleCounterMap() {
	...
	var (
		carCounts  = impl.NewCounterMap[carLabels]("car_count", "The number of cars")
...
func ExampleGaugeMap() {
	...
	var (
		temps          = impl.NewGaugeMap[tempLabels]("temperature", "Temperature, in Fahrenheit")
...
func ExampleHistogramMap() {
	...
	var (
		latencies = impl.NewHistogramMap[latencyLabels](
...
func TestCounter(t *testing.T) {
	c := impl.NewCounter(uuid.New().String(), "")
	c.Add(42)
...
func TestCounterMap(t *testing.T) {
	...
	c := impl.NewCounterMap[labels](uuid.New().String(), "")
...
func TestGauge(t *testing.T) {
	g := impl.NewGauge(uuid.New().String(), "")
	g.Set(42)
	g.Add(42)
	g.Sub(42)
	expect(t, &imetrics.MetricSnapshot{
...
func TestGaugeMap(t *testing.T) {
	type labels struct{ A, B, C string }
	g := impl.NewGaugeMap[labels](uuid.New().String(), "")
...
func TestHistogram(t *testing.T) {
	...
	h := impl.NewHistogram(uuid.New().String(), "", bounds)
	h.Put(42)
	expect(t, &imetrics.MetricSnapshot{
...
func TestHistogramMap(t *testing.T) {
	...
	bounds := []float64{1, 10, 20}
	type labels struct{ A, B, C string }
	c := impl.NewHistogramMap[labels](name, "", bounds)
