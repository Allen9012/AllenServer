...
func init() {
	node.Setup(&ServiceAsync{})
}

// 新建自定义服务TestService1
type ServiceAsync struct {
	service.Service
}

func (slf *ServiceAsync) OnInit() error {
	...
	//会根据任务的数量在最小与最大协程数间动态伸缩
	slf.OpenConcurrent(5, 10, 1000000)
	return nil
}

func (slf *ServiceAsync) OnStart() {
	slf.AfterFunc(time.Second*5, func(*timer.Timer) {
		slf.testAsyncDo()
	})
}

func (slf *ServiceAsync) testAsyncDo() {
	var context struct {
		data int64
	}
