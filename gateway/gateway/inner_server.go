...
func (s *InnerServer) sendMsgToServer(srvID string, cmd uint64, msg interface{}) bool {
	si, ok := s.servers.Load(srvID)
	if !ok {
		return false
	}
...
func (s *InnerServer) HandlerMsg(srvID string, userID uint64, msgID uint64, data []byte) {
	si, ok := s.servers.Load(srvID)
	if !ok {
		return
	}
